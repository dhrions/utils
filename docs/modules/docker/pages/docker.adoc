...
==== Explorer le contenu d'un conteneur

Si le conteneur ne se ferme pas après avoir exécuté la commande :
...
[source, bash]
docker create --name nom_du_conteneur nom_de_l_image
docker cp nom_du_conteneur:/chemin/du/dossier /chemin/local
...
[source, bash]
docker create --name nom_du_conteneur nom_de_l_image
docker export nom_du_conteneur -o contenu_conteneur.tar
docker rm nom_du_conteneur
