...
[source, bash]
make-linux-command install /path/make_linux_command mkcmd --local

[source, bash]
make-linux-command install /path/mkrpo mkrpo --local
...
[source,bash]
----
make-linux-command install <chemin_vers_module> <nom_commande> --local
----
